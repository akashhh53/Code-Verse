...
function ChatAi({ problem }) {
  ...
  return (
    <div className="flex h-full min-h-[30rem] flex-col overflow-hidden rounded-lg border border-base-300 bg-base-100">
      ...
      <div className="flex-1 space-y-4 overflow-y-auto p-4">
        {messages.map((msg, index) => {
          ...
          return (
            <div key={index} className={`flex gap-3 ${isUser ? 'justify-end' : 'justify-start'}`}>
              ...
              <div className={`max-w-[82%] rounded-lg px-4 py-3 text-sm leading-6 shadow-sm ${isUser ? 'bg-primary text-primary-content' : 'bg-base-200 text-base-content'}`}>
                {msg.parts[0].text}
              </div>
              {isUser && (
                <span className="mt-1 flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-base-300 text-base-content">
        ...
        {isSending && (
          <div className="flex items-center gap-3 text-sm text-base-content/60">
            ...
            <span className="inline-flex items-center gap-2 rounded-lg bg-base-200 px-4 py-3">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="border-t border-base-300 bg-base-100 p-4">
        <div className="flex gap-2">
          ...
          />
          <button type="submit" className="btn btn-primary btn-square" disabled={isSending}>
